...
def get_tweet_list(twapi, idlist):
    """
    # Full credits to:
    # chrisinmtown@StackOverflow.
    # Copied from
    # https://stackoverflow.com/questions/28384588/
    ...
    if len(idlist) != len(tweets):
        Logger.warn(
            "get_tweet_list: unexpected response size %d, expected %d",
            len(tweets),
            len(idlist),
        )
    for tweet in tweets:
...
# 28384588/twitter-api-get-tweets-with-specific-id
# Slightly modified to suit my own purpose
