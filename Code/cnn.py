...
from useful_functions import split_data, str2bool, pre_process_data, get_w2vec_model
...
if use_pretrained_vecs is True:
...
# get the w2vec model
w2vec_model, dic_of_words, tokenizer = get_w2vec_model(
...
if use_validation == False:
    ...
    train, validation, test = split_data(
        data, train_test=False, partitioning=(train_fraction, validation_fraction)
    )
    validation_feats = pad_sequences(
        tokenizer.texts_to_sequences(validation["tweets"]), maxlen=max_len_padding
    )
...
# It creates the embedding matrix
# through the use of the vectors
# that has either been trained
# on the GloVe vectors, or the vectors
# trained on the dataset.
# This emb_matrix is later used for
# initializing the weights in the
# embedding layer in the CNN.
...
for w, i in dic_of_words.items():
    try:
        ...
        emb_matrix[i] = np.zeros(emb_dim)  # If the word does not exist,
        # we set it to zeros and consider
        # it non-informative!


# Here begins the network architecture.
# Do note that this is the same architecture
# used by Yoon Kim (2014, see paper).
# Paper is available at
# https://www.aclweb.org/anthology/D14-1181
# Originally, I found the implementation on
# Kaggle, made by Marija Kekic.
# She also uses the exact same network
# architecture as Yoon Kim,
# but for a different problem.
# Her code is available at
# https://www.kaggle.com/marijakekic/cnn-in-keras-with-pretrained-word2vec-weights
# My implementation is inspired by
# her work, although changes
# had to be made to suit this problem.
...
# randomly initialize each word's
# representation, and keep training.

print("train_feats.shape is " + str(train_feats.shape))
inputs = Input(shape=(train_feats.shape[1],))
emb = emb_layer(inputs)

# Reshapes the input to correct
# dimensions to match the input to the convolutional layer.
rshape = Rshape((train_feats.shape[1], emb_dim, 1))(emb)
# Convolutional layer. Kind of the main component of a CNN.
convolutional_neurone0 = Conv2D(
    filters, (3, emb_dim), activation=activation_function, kernel_regularizer=l2(l2_reg)
)(rshape)
convolutional_neurone1 = Conv2D(
    filters, (4, emb_dim), activation=activation_function, kernel_regularizer=l2(l2_reg)
)(rshape)
convolutional_neurone2 = Conv2D(
    filters, (5, emb_dim), activation=activation_function, kernel_regularizer=l2(l2_reg)
)(rshape)
# Max-pooling layers.
# Extract the highest values in
# the output of the convolutions
# retrieved from the first layers.
m_pool0 = MaxPooling2D((train_feats.shape[1] - 3 + 1, 1), strides=(1, 1))(
    convolutional_neurone0
)
m_pool1 = MaxPooling2D((train_feats.shape[1] - 4 + 1, 1), strides=(1, 1))(
    convolutional_neurone1
)
m_pool2 = MaxPooling2D((train_feats.shape[1] - 5 + 1, 1), strides=(1, 1))(
    convolutional_neurone2
)
...
"""
    # Function fetched from:
    # https://stackoverflow.com/questions/
    # 43076609/how-to-calculate-precision-and-recall-in-keras
    # Credits to Christian Skjødt@StackOverflow
    # https://keras.io/metrics/
    # Function is used to be able to
    # aim for precision or recall instead of accuracy.
    # After testing it however,
    # I am uncertain whether it actually works.
    # Thus, the results when using this was not included in the paper.
"""
...
if use_validation == False:
    ...
    model.fit(
        ...
        labels_train,
        epochs=n_epochs,
        validation_data=(validation_feats, validation_labels),
    )
...
class_weights = np.array(
    np.divide(test["class"].value_counts(), np.sum(test["class"].value_counts()))
)
precision_overall = np.sum(np.multiply(precisions, class_weights))
recall_overall = np.sum(np.multiply(recalls, class_weights))
F1_overall = np.divide(
    np.multiply(np.multiply(precision_overall, recall_overall), 2),
    np.add(precision_overall, recall_overall),
)
