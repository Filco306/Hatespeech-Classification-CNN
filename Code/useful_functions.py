...
def pre_process_data(data, remove_stop_words=True, stem=True, include_mentions=False):
    ...
    data.tweet_text.replace(
        r"[\\x]{2}[a-f0-9]{2}", " ", regex=True, inplace=True
    )  # Remove all ascii and non-ascii characters
    data.tweet_text.replace(r"&(\S+)\s?", " ", regex=True, inplace=True)
    data.tweet_text.replace(
        r"[&#]{2}[0-9]{4,6}", " ", regex=True, inplace=True
    )  # Replace all emojis
    ...
    if remove_stop_words == True:
        data.tweets.replace(
            r"\b(?:{})\b".format("|".join(stop_words)), "", regex=True, inplace=True
        )  # Removing stop_words
    data.tweets.replace(
...
def get_w2vec_model(data, use_pretrained_vecs=False, emb_dim=200):
    ...
    if use_pretrained_vecs == False:
        ...
        w2vec_model = KeyedVectors.load_word2vec_format(
            "glove.twitter.27B/glove-twitter-" + str(emb_dim) + ".txt", binary=False
        )
...
def split_data(data, train_test=True, partitioning=0.8, seed=123):
    ...
    if train_test == True:
        ...
        train, validation, test = np.split(
            data.sample(frac=1),
            [
                int(partitioning[0] * len(data)),
                int((partitioning[0] + partitioning[1]) * len(data)),
            ],
        )
...
def print_results(true, preds, file_name):
    ...
    class_weights = np.array(
        np.divide(true.value_counts(), np.sum(true.value_counts()))
    )
